...
public class activityComp extends AppCompatActivity {
    ...
    void onClick(View v){
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 4; j++) {
                if(v.getId() == papan[i][j].letak.getId()){                                                                     // yang ditekan itu sama dengan posisi bidak di papan
                    //turn = white !turn = black
                    if(baris == -1 && kolom == -1) {                                                                            //
                        if (papan[i][j].getBidak().getValue() != 0) {                                                           // jika yang tak tekan tidak kosong
                        ...
                        if(papan[i][j].getStatus() != 0){                                                                                           // masuk sini ketika apa
                            if(papan[i][j].getStatus() == 1){
                                papan[i][j].enPassant = papan[baris][kolom].enPassant;                                                              // iki opo enPassant
